...
static int ilitek_i2c_esd_check(struct i2c_client *client, uint8_t cmd, uint8_t *data, int length);
static int ilitek_i2c_transfer_esd(struct i2c_client *client, struct i2c_msg *msgs, int cnt);
...
// extern mt_eint_unmask(unsigned int line);
// extern mt_eint_mask(unsigned int line);
...
static char DBG_FLAG = 0;	// VV : 0
static char DBG_COR = 0;
...
static const struct i2c_device_id tpd_i2c_id[] = {{"ilitek_i2c",0},{}};
static unsigned short force[] = {0, 0x82, I2C_CLIENT_END,I2C_CLIENT_END};
static const unsigned short * const forces[] = { force, NULL };
//static struct i2c_client_address_data addr_data = { .forces = forces,};
static struct i2c_board_info __initdata i2c_tpd={ I2C_BOARD_INFO("ilitek_i2c", (0x82>>1))};
...
{	
	...
	if(atomic_read(&ilitek_cmd_response) == 0){
		for (i = 0; i < 3; i++) {
			if(ilitek_i2c_esd_check(i2c.client, ILITEK_TP_CMD_GET_PROTOCOL_VERSION, buf, 1) < 0){
				printk("[ilitek esd] %s: ilitek_i2c_read_info \n", __func__);
				if ( i == 2) {
					printk("esd %s: ilitek_i2c_read_info ,  failed three times reset now\n", __func__);
					break;
				}
			}
			else {
        ...
        if(msgs[i].len <= 8){
            ...
            while(count >= 0){
                 ...
                 if((count == 0)&&(ret<0)){
					...
					if(i2c_errcount == 3) {
	                    printk("%s i2c transfer <=8 bytes error 10 times now reset",__func__);
						i2c_errcount = 0;
						ilitek_reset();
					}
                    return ret;
	 ...
	 int length)
{
  ...
  printk("iic address =%x \n",client->addr);
	if (out_use == 0) {
		printk(" [ilitek esd] %s out_use = %d\n", __FUNCTION__, out_use);
	 ...
	 if(ret < 0){
		...
		data[0] = 0x02;
		printk("[ilitek esd] %s  APK USE SO not check out_use = %d\n", __FUNCTION__, out_use);
	}
	 ...
	 int length)
{
	...
	if (out_use == 0) {
		printk(" %s out_use = %d\n", __FUNCTION__, out_use);
		 ret = ilitek_i2c_transfer_esd(client, msgs_cmd, 1);
		 if(ret < 0){
			 //printk(ILITEK_ERROR_LEVEL "%s, i2c read error, ret %dn", __func__, ret);
			 printk(ILITEK_ERROR_LEVEL "%s, i2c read error, ret %d,addr %x \n", __func__, ret,client->addr);
			 return ret;
		 ...
		 msleep(10);
		 ret = ilitek_i2c_transfer_esd(client, msgs_ret, 1);
		 if(ret < 0){
			 //printk(ILITEK_ERROR_LEVEL "%s, i2c read error, ret %dn", __func__, ret);		
			 printk(ILITEK_ERROR_LEVEL "%s, i2c read error, ret %d,addr %x \n", __func__, ret,client->addr);
		}
...
ret = ilitek_upgrade_firmware();
        ...
        if(ret < 0)
        {
		 ...
		 #ifdef USE_MTK_INPUT_REG_MTHD
         if((ret = misc_register(&tpd_misc_device)))
         {
             printk(" iLitek mtk_tpd: tpd_misc_device register failed\n");
             return -1;
         }
		 ...
		 { 
			 ...
			 if(i2c.irq_work_queue){
                ...
                mt_eint_set_sens(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_TYPE);
                    ...
                    mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, EINTF_TRIGGER_FALLING /* CUST_EINT_POLARITY_LOW */, ilitek_i2c_isr, 1);
                    mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
...
static int tpd_local_init(void) 
{
   ...
   hwPowerOn(TP_POWER, VOL_2800, "TP"); 
   // hwPowerOn(mt_POWER_LDO_VGP, VOL_1800, "TP");  
   msleep(50);
   // set INT mode
...
{
    TPD_DMESG("MediaTek iLitek touch panel driver init\n");
...
{
    TPD_DMESG("MediaTek iLitek touch panel driver exit\n");

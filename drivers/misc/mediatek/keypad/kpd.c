...
#ifdef CONFIG_CUSTOM_KERNEL_KPD_MHALL //xmxl@20150519
#define CUST_EINT_MHALL_POLARITY  ((CUST_EINT_MHALL_TYPE==CUST_EINTF_TRIGGER_LOW) ?(0):(1))//CUST_EINT_MHALL_TYPE
...
static void kpd_hallkey_handler(unsigned long data)
{
    ...
    printk(KPD_SAY "kpd_hallkey_state=%d\n",kpd_hallkey_state);
	...
	if(!kpd_input_dev) {
		...
		keycode=(pressed ? KEY_HALL_SLEEP : KEY_HALL_WAKE);
        printk(KPD_SAY "HALL keycode=%d,pressed=%d\n",keycode,pressed);
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	mt_eint_registration(CUST_EINT_MHALL_NUM, CUST_EINT_MHALL_TYPE,kpd_hallkey_eint_handler, 0);
#endif

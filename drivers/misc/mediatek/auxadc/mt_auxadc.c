...
#ifdef CONFIG_MALATA_HW_VERSION
...
#ifdef CONFIG_MALATA_HW_VERSION
static ssize_t show_malata_hw_version(struct device *dev,struct device_attribute *attr, char *buf)
{
//#if defined(CUSTOMER_BLU)
#if defined(CONFIG_MALATA_F6901)
    sprintf(buf,"malata_hw_version=8859C V_3.0\n");
//#else
//    sprintf(buf,"malata_hw_version=8804C V_%d\n",
//			g_hw_version);
//#endif
#else
    sprintf(buf,"malata_hw_version=V%d.0\n", g_hw_version);
#endif

    return strlen(buf);
}

static ssize_t store_malata_hw_version(struct device *dev, struct device_attribute *attr,
				    const char *buf, size_t size)
{
	return size;
}

static DEVICE_ATTR(malata_hw_version, 0664, show_malata_hw_version, store_malata_hw_version);
#endif

#if defined(CONFIG_MALATA_MODEM_VERSION)
static ssize_t show_AUXADC_read_modem_baseband(struct device *dev,struct device_attribute *attr, char *buf)
{
    //read data
    int i = 0, data[4] = {0,0,0,0}, value = 0, chn = 1, val_temp[10];
    char buf_temp[960], ver = 0;
    int res =0;

    res = IMM_auxadc_GetOneChannelValue(chn,data,NULL);
	if(res < 0)
	{
		printk("[adc_driver]: get data error\n");
		sprintf(buf_temp,"AUXADC[%d] Read Error! \n", i);
	}
	else
    {
		printk("[adc_driver]: channel[%d]=%d.%d \n",i,data[0],data[1]);
...
#if defined(CONFIG_MALATA_E1002)
		if((value == 0 || value < 15)){
			ver = 1;
			printk("Modem Baseban is(%d):1+8 \n", value); //baseband:1+8
			sprintf(buf_temp,"1+8 \n");            //baseband:1+8
		}else if((value >= 15) && (value <= 45)){
			ver = 2;
			printk("Modem Baseban is(%d)\n", value); //baseband:undefine
			sprintf(buf_temp,"undefine \n");		 //baseband:undefine
		}else if((value >= 45) && (value <= 75)){
			ver = 3;
			printk("Modem Baseban is(%d):2+4+5 \n", value); //baseband:2+4+5
			sprintf(buf_temp,"2+4+5 \n");        //baseband:2+4+5
		}else if((value >= 75) && (value <= 105)){
			ver = 4;
			printk("Modem Baseban is(%d):1+2+5 \n", value); //baseband:1+2+5
			sprintf(buf_temp,"1+2+5  \n");        //baseband:1+2+5
		}else if((value >= 105) && (value <= 135)){
			ver = 5;
			printk("Modem Baseban is(%d):2+5 \n", value); //baseband:2+5
			sprintf(buf_temp,"2+5 \n");        //baseband:2+5
			...
			ver = 0;
			printk("Modem Baseban is(%d):Unkown \n", value); //baseband:Unkown
			sprintf(buf_temp,"Unkown \n");            //baseband:Unkown
		}
#else
...
static ssize_t store_AUXADC_read_modem_baseband(struct device *dev, struct device_attribute *attr,
				    const char *buf, size_t size)
{
	return size;
}

static DEVICE_ATTR(AUXADC_modem_baseband, 0664, show_AUXADC_read_modem_baseband, store_AUXADC_read_modem_baseband);
#endif
...
#ifdef CONFIG_MALATA_HW_VERSION
	if ((ret = device_create_file(&(dev->dev), &dev_attr_malata_hw_version)) != 0) 
		goto exit;
#endif

#if defined(CONFIG_MALATA_MODEM_VERSION)
	if ((ret = device_create_file(&(dev->dev), &dev_attr_AUXADC_modem_baseband)) != 0) 
		goto exit;
#endif

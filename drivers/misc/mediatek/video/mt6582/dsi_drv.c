...
static void _WaitForEngineNotBusy(void)
{
    ...
    else
    {
        while (DSI_REG->DSI_INTSTA.BUSY || DSI_REG->DSI_INTSTA.CMD_DONE)
        {
            ...
            //enable to enter poweroff state when disconnecting META_3G and avoid to be awaked by interruptible, by Solin 20150203
            if(get_boot_mode() == META_BOOT){
                ret = wait_event_timeout(_dsi_wait_queue,

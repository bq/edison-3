...
kal_int32 battery_meter_get_battery_voltage(kal_bool update)
{
	...
	if (battery_meter_ctrl(BATTERY_METER_CMD_EXT_HW_FG, &arg) == 0){
		...
		printk("[battery_meter_get_battery_voltage] read bq27541 voltage error!!!\n");
	}
#endif
...
kal_int32 battery_meter_get_battery_current(void)
{
	...
	if (battery_meter_ctrl(BATTERY_METER_CMD_EXT_HW_FG, &arg) == 0)
		...
		printk("[battery_meter_get_current] read bq27541 current error!!!\n");
#endif
...
kal_int32 battery_meter_get_battery_percentage(void)
{
	...
	return arg;
		...
		printk("[battery_meter_get_current] read bq27541 battery capacity error!!!\n");
	}
    #endif

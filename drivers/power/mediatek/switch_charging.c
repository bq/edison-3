...
kal_uint32 dynamic_charge_current_by_temp(void)
{
	kal_uint32 ret_current = AC_CHARGER_CURRENT;

	if(BMT_status.temperature < TEMP_LEVEL_1)
		ret_current = AC_CHARGER_CURRENT;
	...
	else if((BMT_status.temperature >= TEMP_LEVEL_5) && (BMT_status.temperature < MAX_CHARGE_TEMPERATURE))
...
void select_charging_curret(void)
{
	if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
			...
			#if defined(CONFIG_MTK_BQ24196_SUPPORT) || defined(CONFIG_MTK_BQ24296_SUPPORT)
				g_temp_CC_value = CHARGE_CURRENT_1100_00_MA;
			#else

...
#Set SHEET_NAME
if($EMMC_SUPPORT eq "yes"){
# xmyxh for data ptgen , reference partition_table_MT8127.xls
#	$SHEET_NAME = "emmc";
	...
	for($row=1;$row < @PARTITION_FIELD;$row++){
		...
		#	$START_FIELD_Byte[$row] = &xls_cell_value($sheet, $row+1, $COLUMN_START,$SHEET_NAME);
			if($COMBO_NAND_SUPPORT eq "yes")
			{
				...
				my $bmtpool= int(($Total_Size * 1024)/$page_p_b/($page/1024)/100)*2;
				if($nand_sandisk)
				{
					$bmtpool = $bmtpool * 2;
				}
        ...
        if($PARTITION_FIELD[$iter] eq "BMTPOOL")
        {
			...
			}else
			{
				if($COMBO_NAND_SUPPORT eq "yes")
				{
					...
					my $bmtpool= int(($Total_Size * 1024)/$p_per_block/($page/1024)/100)*2;
					if($bmtpool > 256)
					{
						my $error_msg="ERROR:Ptgen BMT block count > 256, please decrease BMTPOOL size\n";
						print $error_msg;
						die $error_msg;
					}
					if($nand_sandisk)
		...
		if($MLC_NAND_SUPPORT eq "yes")
		{
			if($DL_TYPE[$index] eq "LOW_PAGE")
			{
				if($PARTITION_FIELD[$index] eq "BMTPOOL")
				{
					...
					my $bmtpool= int(($total_sz * 1024)/$p_per_block/($page_sz/1024)/100)*2;
					print "$bmtpool \n";
					if($nand_sandisk)
					{
						$bmtpool = $bmtpool * 2;

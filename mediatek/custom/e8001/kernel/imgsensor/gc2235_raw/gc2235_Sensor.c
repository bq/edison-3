...
static void GC2235_Sensor_Init(void)
{
	...
	GC2235_write_cmos_sensor(0x17, 0x16);//14 //[0]mirror [1]flip
...
#endif
    printk("zrc enter  GC2235Control\n");
	switch (ScenarioId)
	{
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
...
UINT32 GC2235MIPI_SetTestPatternMode(kal_bool bEnable)
{
	...
	if(bEnable) 
	{
	   ...
	   GC2235_write_cmos_sensor(0xb4,0x40);
	   GC2235_write_cmos_sensor(0xb5,0x40);
	   GC2235_write_cmos_sensor(0xb1,0x40);
       GC2235_write_cmos_sensor(0x03,0x00);
       GC2235_write_cmos_sensor(0x04,0x00);
       GC2235_write_cmos_sensor(0x8b,0x10);
        
	}
	else        
	{

        GC2235_write_cmos_sensor(0x8b,0x00);
	}

...
/*[A] add for distinguishing camera module by xmcwy 20140728 */
int g_module_id = 0x2b;
...
int OV5648MIPI_check_otp_wb(int index)
{
    int flag, i;
    int rg, bg;
    ...
    if (index == 1){
        ...
        SENSORDB("xmcwy_camera_otp index = %d flag = %d rg = 0x%x bg = 0x%x \n", index, flag, rg, bg);
    }
    else if (index == 2){
        ...
        SENSORDB("xmcwy_camera_otp index = %x flag = %x rg = 0x%x bg = 0x%x \n", index, flag, rg, bg);
    }
    else if (index == 3){
    ...
    if (flag) {
        SENSORDB("xmcwy_camera_otp index = %d is invalid data group \n", index);
        return 1;
    }
    else
    {
        if (rg == 0  || bg == 0){
            SENSORDB("xmcwy_camera_otp index = %d is empty data group \n", index);	
            return 0;
        }
        else{
            SENSORDB("xmcwy_camera_otp index = %d is valid data group \n", index);
            return 2;
        }
...
//index:index of otp group.(1,2,3)
//return:	0.
int OV5648MIPI_read_otp_wb(int index, struct OV5648MIPI_otp_struct *otp_ptr)
{
    int i, temp;
    ...
    if (index == 1){
        ...
        SENSORDB("xmcwy_camera_otp module_integrator_id = 0x%x \n",(*otp_ptr).module_integrator_id);
        SENSORDB("xmcwy_camera_otp lens_id = 0x%x \n",(*otp_ptr).lens_id);
        SENSORDB("xmcwy_camera_otp rg_ratio = 0x%x \n",(*otp_ptr).rg_ratio);
        SENSORDB("xmcwy_camera_otp bg_ratio = 0x%x \n",(*otp_ptr).bg_ratio);
        SENSORDB("xmcwy_camera_otp light_rg = 0x%x \n",(*otp_ptr).light_rg);
        SENSORDB("xmcwy_camera_otp light_bg = 0x%x \n",(*otp_ptr).light_bg);
        SENSORDB("xmcwy_camera_otp user_data[0]  = 0x%x \n",(*otp_ptr).user_data[0] );
        SENSORDB("xmcwy_camera_otp user_data[1] = 0x%x \n",(*otp_ptr).user_data[1]);
    }
    else if (index == 2){
...
//reutrn 0
int OV5648MIPI_update_wb_gain(int R_gain, int G_gain, int B_gain)
{
...
int RG_Ratio_Typical = RG_Typical_1;
int BG_Ratio_Typical = BG_Typical_1;
...
{	
    ...
    SENSORDB("xmcwy_camera_otp default RG_Ratio_Typical = 0x%x BG_Ratio_Typical = 0x%x \n", \
    ...
    for(i = 1; i <= 3; i++){
        ...
        temp = OV5648MIPI_check_otp_wb(i);
    	 ...
    	 if(temp == 2){ 	
	     otp_index = i;
	     SENSORDB("xmcwy_camera_otp otp_index = %d is used  \n",otp_index);
            break;		
        }
    }
    ...
    if(i > 3){
        ...
        g_module_id = 0xff;//0xff=unison_v1.0 without OTP by xmcwy
        SENSORDB("xmcwy_camera_otp g_module_id = 0x%x \n",g_module_id);
	 SENSORDB("xmcwy_camera_otp no OTP data !!! \n");
        return 1;
    }
    ...
    /*[A] add for distinguishing camera module by xmcwy 20140728 */
    g_module_id = current_otp.module_integrator_id;//0x00=unison_v2.0 0x10=seasons  
    SENSORDB("xmcwy_camera_otp g_module_id = 0x%x \n",g_module_id);
    ...
    SENSORDB("xmcwy_camera_otp unison RG_Ratio_Typical = 0x%x BG_Ratio_Typical = 0x%x \n", \
		RG_Ratio_Typical, BG_Ratio_Typical);
    }
    else if(g_module_id == 0x10){

    RG_Ratio_Typical = RG_Typical_2;
    BG_Ratio_Typical = BG_Typical_2;
    SENSORDB("xmcwy_camera_otp seasons RG_Ratio_Typical = 0x%x BG_Ratio_Typical = 0x%x \n", \
		RG_Ratio_Typical, BG_Ratio_Typical);
    }
#endif
    /*[E] add for distinguishing camera module by xmcwy 20140728 */
    ...
    //write  wb gain to sensor register
...
static void OV5648MIPI_Preview_Setting(void)
{
    ...
    /*[A] fix sensor orientation differ by xmcwy 20140728 */
    if((g_module_id == 0x10)||(g_module_id == 0xff)){
        OV5648MIPI_Set_Mirror_Flip(IMAGE_HV_MIRROR);
    }
    /*[E] fix sensor orientation differ by xmcwy 20140728 */

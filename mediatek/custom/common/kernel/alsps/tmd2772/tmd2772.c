...
static int tmd2771_read_prox_mean(struct i2c_client *client);
...
static DEFINE_SEMAPHORE(tmd2772_enabling_ps_mutex);//xmxl@20140526
/*----------------------------------------------------------------------------*/
...
static u8 tmd2771_read_id(struct i2c_client *client)
{
	u8 buffer[1];
	int res=0;
	
	buffer[0] = TMD2771_CMM_ID;
...
static long tmd2771_enable_als(struct i2c_client *client, int enable)
{
		...
		if(enable)
		{
			...
			atomic_set(&obj->als_deb_on, 1);
			atomic_set(&obj->als_deb_end, jiffies+atomic_read(&obj->als_debounce)/(1000/HZ));
			APS_DBG("tmd2771_enable_als power on\n");
		}
		else
		{	
			if(testbit_PS){
			...
			/*Lenovo-sw chenlj2 add 2011-06-03,modify ps_deb_on to als_deb_on */
			atomic_set(&obj->als_deb_on, 0);
			APS_DBG("tmd2771_enable_als power off\n");//tmd2771 power
		}
		#endif
...
static long tmd2771_enable_ps(struct i2c_client *client, int enable)
{
	...
	if(enable)
	{
		...
		atomic_set(&obj->ps_deb_on, 1);
		atomic_set(&obj->ps_deb_end, jiffies+atomic_read(&obj->ps_debounce)/(1000/HZ));
		APS_DBG("tmd2771_enable_ps, tmd2771 power on\n");
        
		//temp_ps_mean = tmd2771_read_prox_mean(client);
		//if(/*temp_ps_mean<200 ||*/ 750<=temp_ps_mean)
		{
			ps_cali.valid=0;
			tmd2771_read_data_for_cali(obj->client,&ps_cali);//recalibrate 
		}
		/*else
		{
			ps_cali.close	      = temp_ps_mean + 120; 
			ps_cali.far_away = temp_ps_mean  + 70;
			ps_cali.valid=1;
			APS_LOG("tmd2771_enable_ps: close  = %d,far_away = %d,valid = %d",ps_cali.close,ps_cali.far_away,ps_cali.valid);
		}*/
		...
		if(0 == obj->hw->polling_mode_ps)
		{
			...
			{
				databuf[0] = TMD2771_CMM_INT_LOW_THD_LOW;	
				databuf[1] = (u8)(atomic_read(&tmd2771_obj->ps_thd_val_low) & 0x00FF);//750
				res = i2c_master_send(client, databuf, 0x2);
				if(res <= 0)
				{
					goto EXIT_ERR;
				...
				databuf[0] = TMD2771_CMM_INT_LOW_THD_HIGH;	
				databuf[1] = (u8)((atomic_read(&tmd2771_obj->ps_thd_val_low) & 0xFF00) >> 8);//750
				res = i2c_master_send(client, databuf, 0x2);
				if(res <= 0)
				{
					goto EXIT_ERR;
				...
				databuf[0] = TMD2771_CMM_INT_HIGH_THD_LOW;	
				databuf[1] = (u8)( atomic_read(&tmd2771_obj->ps_thd_val_high) & 0x00FF);//900
				res = i2c_master_send(client, databuf, 0x2);
				if(res <= 0)
				{
					goto EXIT_ERR;
				...
				databuf[0] = TMD2771_CMM_INT_HIGH_THD_HIGH; 
				databuf[1] = (u8)( (atomic_read(&tmd2771_obj->ps_thd_val_high) & 0xFF00) >> 8);//900
				res = i2c_master_send(client, databuf, 0x2);
				if(res <= 0)
				{
					goto EXIT_ERR;
	...
	if(testbit_ALS){
		...
		atomic_set(&obj->ps_deb_on, 0);
		APS_DBG("tmd2771_enable_ps, tmd2771 power off\n");
...
int tmd2771_setup_eint(struct i2c_client *client)
{
	...
	mt_eint_mask(CUST_EINT_ALS_NUM); //mt_eint_unmask(CUST_EINT_ALS_NUM);  
    return 0;
}
...
static int tmd2771_init_client_for_cali(struct i2c_client *client)
{
   ...
   //---------read id:
	...
	databuf[1] = TMD2771_CMM_PPCOUNT_VALUE;//0x0B;
	res = i2c_master_send(client, databuf, 0x2);
	if(res <= 0)
	{
		goto EXIT_ERR;
	...
	databuf[1] = TMD2771_CMM_CONTROL_VALUE;//0x20;//0x20;//0xE0;
	res = i2c_master_send(client, databuf, 0x2);
	if(res <= 0)
	{
		goto EXIT_ERR;
	...
	databuf[0]=TMD2771_CMM_OFFSET;//xmxl:clear offset
	databuf[1]= 0x00;
	res= i2c_master_send(client, databuf, 0x2);
	if(res<= 0)
	{
...
static int tmd2771_init_client(struct i2c_client *client)
{
	...
	databuf[1] = TMD2771_CMM_OFFSET_VALUE;//0x20;//0x20;//0xE0;
	res = i2c_master_send(client, databuf, 0x2);
	if(res <= 0)
	{
		goto EXIT_ERR;
...
long tmd2771_read_ps(struct i2c_client *client, u16 *data)
{
    ...
    //printk("tmd2771_read_ps IN \n");
		
	if(client == NULL)
	{
		printk("tmd2771 CLIENT CANN'T EQUL NULL\n");
		return -1;
	}
	...
	*data = ps_value_low[0] | (ps_value_high[0]<<8);
	//printk("tmd2771 ps_data=%d, low:%d  high:%d;\n", *data, ps_value_low[0], ps_value_high[0]);
	return 0;    
...
static int tmd2771_get_ps_value(struct tmd2771_priv *obj, u16 ps)
{
	...
	//return 1;//xmxl: "return 1" disables proximity function temporarily
...
static void tmd2771_eint_work(struct work_struct *work)
{
	...
	else
	{
		...
		sensor_data.status =ps_cali.valid ? SENSOR_STATUS_ACCURACY_MEDIUM : SENSOR_STATUS_ACCURACY_LOW;
		APS_ERR("ps=%d,als=%d,values[0]=%d,status=%d!\n",obj->ps,obj->als,sensor_data.values[0],sensor_data.status);
...
#define READ_PS_NUM 5
static int tmd2771_read_prox_mean(struct i2c_client *client)
{
	u16 prox_sum = 0,prox_mean = 0,prox_psdata = 0,psdata[READ_PS_NUM];	
	u8 i,err,validcnt=0;

	//mdelay(10);	
	for(i = 0,validcnt=0;i<READ_PS_NUM;i++)
	{
		mdelay(15);//10,15
		tmd2771_read_ps(client,&prox_psdata);
		psdata[i]=	prox_psdata;
		//APS_LOG("prox_psdata[%d] = %d\t",i,prox_psdata);
		//if(prox_psdata>5){
		validcnt++;
		prox_sum +=prox_psdata;
		//}
	 }
	 prox_mean =  prox_sum/validcnt;

	//if(invalidcnt>0)
	//    tmd2771_report_ps_error();
	printk("tmd2771 prox_mean = %d; %d,%d,%d,%d,%d\n",prox_mean,psdata[0],psdata[1],psdata[2],psdata[3],psdata[4]);
...
static u8 tmd2771_calculate_offset(u16 prox)
{
   u8 offset=0;
   int offsetcnt=0;
   prox=(prox>1023) ? 1023 : prox;
   
   if(prox<400){
   	offsetcnt=((400-prox)/((TMD2771_CMM_PPCOUNT_VALUE/8)*5));
	offsetcnt= (offsetcnt>2) ?  (offsetcnt-2) : 0;
   	offset=0x80|offsetcnt;
   }
   else{
   	offsetcnt=((prox-400)/((TMD2771_CMM_PPCOUNT_VALUE/8)*5));
	offsetcnt= (offsetcnt>2) ?  (offsetcnt-2) : 0;
   	offset=(~0x80) & offsetcnt;
   }
...
{
     	int i=0 ,err = 0;
	u8 buffer[2]={0,0},offsetValid;
	u16 prox_mean_clai = 0,prox_mean_clai_noOffset = 0;

    //return -1;//xmxl "return -1" disables auto-cali  proximity
        

	tmd2771_init_client_for_cali(client);

	mdelay(5);//5ms		
	prox_mean_clai = tmd2771_read_prox_mean(client);//get prox_mean
	prox_mean_clai_noOffset=prox_mean_clai;

	
	buffer[1]=tmd2771_calculate_offset(prox_mean_clai);
	offsetValid=buffer[1];
	
	//if(0!=buffer[1])
	{
		buffer[0]=TMD2771_CMM_OFFSET;
		//buffer[1]= 0x00|0x81;//0x7f;
		...
		mdelay(5);//5ms
		prox_mean_clai = tmd2771_read_prox_mean(client);
	}

	if(/*prox_mean_clai<200||*/750<=prox_mean_clai){ 
		goto EXIT_FAILL;//cali failed
	}

EXIT_OK: // 1.offset is effective
	#if  defined(CUSTOMER_S450)
	ps_data_cali->close = prox_mean_clai+65;//120;//60;
	ps_data_cali->far_away=prox_mean_clai+50;//70;//40;
	#else
	ps_data_cali->close = prox_mean_clai+120;//60;
	ps_data_cali->far_away=prox_mean_clai+70;//40;
	#endif
	ps_data_cali->valid = 1;
	
	prox_mean_clai= (prox_mean_clai>prox_mean_clai_noOffset) ? (prox_mean_clai-prox_mean_clai_noOffset) : (prox_mean_clai_noOffset-prox_mean_clai);
	APS_ERR("ok: close=%d,far_away=%d,valid=%d;offsetcnt=0x%x,offset=%d,delat=%d\n",ps_data_cali->close,ps_data_cali->far_away,ps_data_cali->valid,offsetValid,((offsetValid&0x7F)*20),prox_mean_clai);
        return 0;
EXIT_FAILL: // 1.offset is ineffective  2.raw ps data is too big
	buffer[0]=TMD2771_CMM_OFFSET;
	buffer[1]= 0x00;
	err= i2c_master_send(client, buffer, 0x2);
	ps_data_cali->close = (prox_mean_clai_noOffset+120)>1023 ? atomic_read(&tmd2771_obj->ps_thd_val_high) : (prox_mean_clai_noOffset+120);
	ps_data_cali->far_away= (ps_data_cali->close-50);
	ps_data_cali->valid = 0;
	
	prox_mean_clai= (prox_mean_clai>prox_mean_clai_noOffset) ? (prox_mean_clai-prox_mean_clai_noOffset) : (prox_mean_clai_noOffset-prox_mean_clai);
	err=  -1;
	APS_ERR("error: close  = %d,far_away = %d,valid = %d;offsetcnt=0x%x,offset=%d,delta=%d\n",ps_data_cali->close,ps_data_cali->far_away,ps_data_cali->valid,offsetValid,((offsetValid&0x7F)*20),prox_mean_clai);
	return err;
...
/*
		...
		case ALSPS_GET_PS_RAW_DATA_FOR_CALI:
			...
			err = tmd2771_read_data_for_cali(obj->client,&ps_cali);
			if(err)
			{
			   goto err_out;
			}
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			{				
				value = *(int *)buff_in;
				if(down_interruptible(&tmd2772_enabling_ps_mutex)){//xmxl@20140526
					APS_ERR("PS SENSOR_ENABLE can't get semaphore\n"); 
					return -EFAULT;
				}
				//APS_ERR("SENSOR_ENABLE %d\n", value); 
				if(value)
				{
					//tmd2771_read_data_for_cali(obj->client,&ps_cali);
					if(err = tmd2771_enable_ps(obj->client, 1))
					{
						APS_ERR("enable ps fail: %d\n", err); 
						up(&tmd2772_enabling_ps_mutex);//xmxl@20140526
						return -1;
					}
					set_bit(CMC_BIT_PS, &obj->enable);
					#if 0	
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				if(test_bit(CMC_BIT_PS, &obj->enable)){//xmxl@20140526
					sensor_data->values[0]=obj->als_last;
				}
				else{	
					...
					#if 1// defined(MALATA_E6901)
					sensor_data->values[0] = obj->als;//(obj->als<1) ? obj->als : (obj->als+50);
					#else
				...
				obj->als_last=sensor_data->values[0];//xmxl@20140526
...
static int tmd2771_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ps_cali.close = obj->hw->ps_threshold_high;
	ps_cali.far_away=obj->hw->ps_threshold_low;
	...
	//tmd2771_read_data_for_cali(client,&ps_cali);//xmxl mask it
		   
	if(err = tmd2771_init_client(client))
	{
		goto exit_init_failed;
	}
	APS_LOG("tmd2771_init_client() OK! DEVICE ID=0x%x\n",tmd2771_read_id(client));
...
static int __init tmd2771_init(void)
{
	APS_FUN();
	struct alsps_hw *hw = get_cust_alsps_hw();
	APS_LOG("%s:TMD2772 i2c_number=%d\n", __func__,hw->i2c_num); 

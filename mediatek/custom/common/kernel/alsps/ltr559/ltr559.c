 ...
 */
#if defined(MALATA_Q5)
static int ps_trigger_high = 750;				//yaoyaoqin: trigger up threshold
static int ps_trigger_low = 650;				//yaoyaoqin: trigger low threshold
#else
...
int ltr559_setup_eint(struct i2c_client *client)
{
	...
	mt_eint_registration(CUST_EINT_ALS_NUM, CUST_EINT_ALS_POLARITY, ltr559_eint_func, 0);
	mt_eint_unmask(CUST_EINT_ALS_NUM);	

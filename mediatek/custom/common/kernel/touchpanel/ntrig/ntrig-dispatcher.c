...
int create_single_touch(struct _ntrig_bus_device *dev, __u8 sensor_id)
{
    ...
    __set_bit(ABS_PRESSURE, input_device->absbit);
	...
	for (i = 0; i < fingers_num; i++) {
		...
		if (fingers[i].removed) {
			...
			*/
			...
			printk("ysl x:%d, y:%d\n", fingers[i].x_coord, fingers[i].y_coord);
		}
	}
	input_sync(input);
...
//			input_report_key(input, BTN_TOOL_PEN, 0x00);
			...
			/* ntrig_dbg("%s: sent  ABS_MT_TOUCH_MAJOR,0x0\n",
			 *	__func__); */
//			printk("ysl up finger:%d, x:%d, y:%d, index:%d\n", i, fingers[i].x_coord, fingers[i].y_coord, index);
			mark_removed_finger_map_index(index);
		} else {
...
//			printk("ysl down finger:%d, x:%d, y:%d, index:%d\n", i, fingers[i].x_coord, fingers[i].y_coord, index);

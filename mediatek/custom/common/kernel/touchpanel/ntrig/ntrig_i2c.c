...
#define GTP_DMA_MAX_TRANSACTION_LENGTH 4096
static u8 *gpDMABuf_va = NULL;
static u32 gpDMABuf_pa = NULL;
static struct ntrig_i2c_mod_privdata *ntrig_priv;
...
int ntrig_i2c_read(struct i2c_client *client, short reg, u8* buf, short len)
{
	...
	struct i2c_msg msg[2] = {
			{ client->addr, client->flags, 2, (u8 *)&reg, 400 },
			{ client->addr, client->flags | I2C_M_RD, len, gpDMABuf_pa, 400, I2C_DMA_FLAG}
	};

	ntrig_dbg(
		"in %s, register is %hd, length is %hd\n", __func__, reg, len);

	if (len >= GTP_DMA_MAX_TRANSACTION_LENGTH) {
		printk("i2c read error len:%d\n", len);
	}
	
	ret = ntrig_i2c_transfer(client, msg, 2);
	if (ret == 0)
...
int ntrig_i2c_fast_read(struct i2c_client *client, u8* buf, short len)
{
	...
	ret = ntrig_i2c_transfer(client, msg, 1);
	if (ret == 0)
		memcpy(buf, gpDMABuf_va, len);
...
int ntrig_i2c_write(struct i2c_client *client, u8* buf, short len)
{
	...
	struct i2c_msg msg[1] = {
			{ client->addr, client->flags, len, gpDMABuf_pa, 400, I2C_DMA_FLAG}
	};
...
{
	struct ntrig_i2c_mod_privdata *privdata = ntrig_priv;
	ntrig_dbg("in %s\n", __func__);
...
static int touch_event_handler(void *unused)
 {
	...
	do {
		ntrig_dbg("%s: gpio line is low (active interrupt) gpio:%d\n",__func__, mt_get_gpio_in(GPIO_CTP_EINT_PIN));
		ntrig_dbg("in %s, before down\n", __func__);
		/*critical section: i2c transfer*/
		down(&privdata->i2c_lock);
		ntrig_dbg("in %s, after down\n", __func__);
		...
		/*critial section end*/
		up(&privdata->i2c_lock);
		ntrig_dbg("in %s, after up gpio:%d\n", __func__, mt_get_gpio_in(GPIO_CTP_EINT_PIN));
	} while (mt_get_gpio_in(GPIO_CTP_EINT_PIN) == 0);

	handle_delayed_hb_reply(privdata);
	mt_eint_unmask(g_i2c_client->irq);
 }while(!kthread_should_stop());
 	ntrig_dbg("thread out\n");
...
int ntrig_i2c_mod_init(void)
{
	...
	ntrig_priv = pdata = kzalloc(sizeof(struct ntrig_i2c_mod_privdata), GFP_KERNEL);
	g_pdata = pdata;
	if (pdata == NULL) {
...
//			flags, DRIVER_NAME, pdata);
	mt_eint_registration(g_i2c_client->irq, flags, i2c_irq, 0);
	mt_eint_unmask(g_i2c_client->irq);
	mt_set_gpio_pull_enable(GPIO_CTP_EINT_PIN, GPIO_PULL_ENABLE);
...
//	}
	...
	*/
	...
	i2c_reset_counters();
	thread = kthread_run(touch_event_handler, 0, "ntrig");
	 if (IS_ERR(thread))
	{

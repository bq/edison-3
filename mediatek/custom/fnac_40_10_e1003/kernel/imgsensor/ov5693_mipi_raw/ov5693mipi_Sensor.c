...
static int ov5693_otp_data_year=0;
static int ov5693_otp_data_month=0;
static int ov5693_otp_data_day=0;
...
int read_otp_wb(int index, struct otp_struct *otp_ptr)
{
	...
	printk("honghaishen_ov5693 year is %d, month is %d, day is %d",(*otp_ptr).production_year,(*otp_ptr).production_month,(*otp_ptr).production_day);
	...
	(*otp_ptr).light_rg = (OV5693MIPI_read_cmos_sensor(address + 8) <<2) + ((temp>>2) &
	0x03);
...
int read_otp_lenc(int index, struct otp_struct *otp_ptr)
{
	...
	for(i=0;i<15;i++)
	{
		(* otp_ptr).lenc[i]=OV5693MIPI_read_cmos_sensor(address);
		address++;
	}
	// clear otp buffer
	for (i=0;i<16;i++)
...
int read_otp_VCM(int index, int code, struct otp_struct * otp_ptr)
{
	...
	// read flag
	address = 0x3d00 + (index-1)*4 + code*2;
	vcm = OV5693MIPI_read_cmos_sensor(address);
	vcm = (vcm & 0x03) + (OV5693MIPI_read_cmos_sensor(address+1)<<2);
	if(code==1)
	{
		(* otp_ptr).VCM_end = vcm;
	}
...
int update_lenc(struct otp_struct * otp_ptr)
{
	...
	for(i=0;i<62;i++)
	{
		OV5693MIPI_write_cmos_sensor(0x5800 + i, (*otp_ptr).lenc[i]);
	}
...
int read_otp_date()
{
	...
	OV5693MIPI_write_cmos_sensor(0x0100, 0x01);
	msleep(20);
	for(i=1;i<=3;i++)
	{
		temp = check_otp_wb(i);
		...
		{
			otp_index = i;
			printk("honghaishen_ov5693 read date index is %d \n",otp_index);
			break;
		}
	}
	if (i>3)
	...
	printk("honghaishen_ov5693 camera read data fail \n");
		return 1;
	}
	read_otp_wb(otp_index, &current_otp);
	OV5693MIPI_write_cmos_sensor(0x0100, 0x00);
	printk("honghaishen_ov5693 read hou 1 year is %d, month is %d, day is %d",current_otp.production_year,current_otp.production_month,current_otp.production_day);
	ov5693_otp_data_year=current_otp.production_year;
	ov5693_otp_data_month=current_otp.production_month;
	ov5693_otp_data_day=current_otp.production_day;
	printk("honghaishen_ov5693 read hou 2 year is %d, month is %d, day is %d",ov5693_otp_data_year,ov5693_otp_data_month,ov5693_otp_data_day);
	return 0;
...
int update_otp_wb()
{
	...
	for(i=1;i<=3;i++)
	{
		...
		{
			otp_index = i;
			printk("honghaishen_ov5693 wb index is %d \n",otp_index);
			break;
		}
	}
	if (i>3)
	...
	printk("honghaishen_ov5693 wb rg is %d , bg is %d ",rg,bg);
	printk("honghaishen_ov5693 wb rg_type is %d , bg_type is %d ",rg_type,bg_type);
	//calculate G gain
	//0x400 = 1x gain
...
int update_otp_lenc()
{
	...
	for(i=1;i<=3;i++)
	{
		...
		{
			otp_index = i;
			printk("honghaishen_ov5693 lenc index is %d \n",otp_index);
			break;
		}
	}
	if (i>3)
	{
		...
		printk("honghaishen_ov5693 lenc index is no \n");
		return 1;
	}
...
int update_blc_ratio()
{
	int K;
	int temp;
	OV5693MIPI_write_cmos_sensor(0x3d84, 0xdf);
	OV5693MIPI_write_cmos_sensor(0x3d81, 0x01);
	msleep(5);
	K = OV5693MIPI_read_cmos_sensor(0x3d0b);
	if (K != 0)
	{
		if (K >= 0x15 && K <= 0x40)
		{
			// auto load mode
			temp = OV5693MIPI_read_cmos_sensor(0x4008);
			temp &= 0xfb;
			OV5693MIPI_write_cmos_sensor(0x4008, temp);
			temp = OV5693MIPI_read_cmos_sensor(0x4000);
			temp &= 0xf7;
			OV5693MIPI_write_cmos_sensor(0x4000, temp);
			return 2;
		}
	}
	K = OV5693MIPI_read_cmos_sensor(0x3d0a);
	if (K >= 0x10 && K <= 0x40)
	{
		// manual load mode
		OV5693MIPI_write_cmos_sensor(0x4006, K);
		temp = OV5693MIPI_read_cmos_sensor(0x4008);
		temp &= 0xfb;
		OV5693MIPI_write_cmos_sensor(0x4008, temp);
		temp = OV5693MIPI_read_cmos_sensor(0x4000);
	...
	{
		// set to default
		OV5693MIPI_write_cmos_sensor(0x4006, 0x20);
		temp = OV5693MIPI_read_cmos_sensor(0x4008);
		temp &= 0xfb;
		OV5693MIPI_write_cmos_sensor(0x4008, temp);
		temp = OV5693MIPI_read_cmos_sensor(0x4000);
...
static void OV5693MIPI_Sensor_Init(void)
{
    ...
    */
    ...
    OV5693MIPI_write_cmos_sensor(0x309a,0x02);  //PLL  0x02(20140110)
    ...
    OV5693MIPI_write_cmos_sensor(0x30b3,0x6a);  //PLL 50 64 6c 0x6a(20140110)
    ...
    OV5693MIPI_write_cmos_sensor(0x3600,0xbc);  
    ...
    OV5693MIPI_write_cmos_sensor(0x3620,0x44);
    OV5693MIPI_write_cmos_sensor(0x3621,0xb5);
    OV5693MIPI_write_cmos_sensor(0x3622,0x0c);
...
UINT32 OV5693MIPIOpen(void)
{
	...
	read_otp_date();
	printk("honghaishen_ov5693 otp year is %d, month is %d, day is %d \n",ov5693_otp_data_year,ov5693_otp_data_month,ov5693_otp_data_day);
	if( (14*13*32 + 7*32 + 17) > (ov5693_otp_data_year*13*32 + ov5693_otp_data_month * 32 + ov5693_otp_data_day) )
		is_ov5693=1;
	if( ((14*13*32 + 11*32 + 14) > (ov5693_otp_data_year*13*32 + ov5693_otp_data_month * 32 + ov5693_otp_data_day) )
		&& ((14*13*32 + 10*32 + 28) < (ov5693_otp_data_year*13*32 + ov5693_otp_data_month * 32 + ov5693_otp_data_day) ) )
		is_ov5693=1;
	printk("honghaishen_ov5693 is_ov5693 is %d \n",is_ov5693);
	if(is_ov5693==0)
	{
		return ERROR_SENSOR_CONNECT_FAIL;
	}
...
UINT32 OV5693GetSensorID(UINT32 *sensorID) 
{
	...
	if( (14*13*32 + 7*32 + 17) > (ov5693_otp_data_year*13*32 + ov5693_otp_data_month * 32 + ov5693_otp_data_day) )
		is_ov5693=1;
	if( ((14*13*32 + 11*32 + 14) > (ov5693_otp_data_year*13*32 + ov5693_otp_data_month * 32 + ov5693_otp_data_day) )
		&& ((14*13*32 + 10*32 + 27) < (ov5693_otp_data_year*13*32 + ov5693_otp_data_month * 32 + ov5693_otp_data_day) ) )
		is_ov5693=1;
	printk("honghaishen_ov5693 is_ov5693 is %d \n",is_ov5693);
	if(is_ov5693==0)
	{

...
const CALIBRATION_LAYOUT_STRUCT CalLayoutTbl[MAX_CALIBRATION_LAYOUT_NUM]=
{
	{//CALIBRATION_LAYOUT_SLIM_LSC1 without Defect //data sheet of excel : "Slim"
		0x00000001, 0xff000b02, CAM_CAL_SINGLE_EEPROM_DATA,
...
UINT32 DoCamCalSingleLsc(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
    ...
    else
    {
        ...
        else
        {
			...
			//memcpy(&pCamCalData->SingleLsc.LscTable, &tmp_LSC, sizeof(tmp_LSC));
			//memcpy(&tmp_LSC, &pCamCalData->SingleLsc.LscTable, sizeof(pCamCalData->SingleLsc.LscTable));
			if(pCamCalData->SingleLsc.LscTable.Data[BlockSize-1]==0xff && pCamCalData->SingleLsc.LscTable.Data[BlockSize-2]==0x00 && pCamCalData->SingleLsc.LscTable.Data[BlockSize-3]==0x02 && pCamCalData->SingleLsc.LscTable.Data[BlockSize-4]==0x01)
			{
				...
				for(Count = 0; Count<=383; Count++)
					pCamCalData->SingleLsc.LscTable.MtkLcsData.CapTable[383-Count] = pCamCalData->SingleLsc.LscTable.Data[Count];
				...
				//for(Count2 = 0; Count2<=BlockSize; Count2++)
					//CAM_CAL_LOG_IF(dumpEnable,"Data[%d] = 0x%x \n",Count2, pCamCalData->SingleLsc.LscTable.Data[Count2]);
				//#endif	
 
				//for(Count=0; Count<380; Count++)
							//pCamCalData->SingleLsc.LscTable.MtkLcsData.CapTable[Count] = pCamCalData->SingleLsc.LscTable.Data[BlockSize-69-Count];
				...
				*/
				...
				for(Count2 = 0; Count2<=383; Count2++)
					CAM_CAL_LOG_IF(dumpEnable,"CapTable[%d] = 0x%x \n",Count2, pCamCalData->SingleLsc.LscTable.MtkLcsData.CapTable[Count2]);
				CAM_CAL_LOG_IF(dumpEnable,"======================cam_cal_lsc==================\n");
				#endif	
			}
			else

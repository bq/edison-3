    ...
    )
{
	...
	if (prGlueInfo->prAdapter->rWifiVar.prP2pFsmInfo->rChnlReqInfo.NFC_BEAM) {
		printk("TxL[%d]\n",prSkb->len);
		if((prSkb->len<100) && (prSkb->len>60)) {
				PUCHAR pcPtr=(PUCHAR)prSkb->data;
			pcPtr+=0x26;
					printk(MACSTR":%02x:%02x\n",MAC2STR(pcPtr),*(pcPtr+0x6),*(pcPtr+0x7));

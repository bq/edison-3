...
#ifdef BATTERY_THERMAL_CHECK_POWER_ON
...
#define G_CHARGER_TYPE_CHECK_MAX 3 //xmxl@20140417: check charger type;  0-->not check , 1--> check 1 time
int g_charger_type_check_cnt=G_CHARGER_TYPE_CHECK_MAX;
...
int  save_thermal_status(int state)
{
	...
	if(fd < 0){
		battery_xlog_printk(BAT_LOG_CRTI, "Open /dev/misc failed\n");
		ret = 0;
	}else{
		if(0 == state){
    ...
    union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_ONLINE:                           
		...
		battery_xlog_printk(BAT_LOG_CRTI, "[ac_get_property] data->AC_ONLINE %d\n", data->AC_ONLINE);
        break;
    default:
        ret = -EINVAL;
        break;
    ...
    union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_ONLINE:     
        ...
        #else
        val->intval = data->USB_ONLINE;
		battery_xlog_printk(BAT_LOG_CRTI, "[usb_get_property] data->USB_ONLINE %d\n", data->USB_ONLINE);
        #endif        
        break;
    default:
        ret = -EINVAL;
...
static void mt_battery_notify_VBatTemp_check(void)
{
...
#ifdef MAX_CHARGE_TEMPERATURE_NOTIFY
	if(BMT_status.temperature >= MAX_CHARGE_TEMPERATURE_NOTIFY)
#else
	if(BMT_status.temperature >= MAX_CHARGE_TEMPERATURE)
#endif
    {
    ...
    battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] BATTERY_NOTIFY_CASE_0002_VBATTEMP (%x), batt_temp: %d\n", g_BatteryNotifyCode,BMT_status.temperature);
...
#else
    if((BMT_status.charger_type == CHARGER_UNKNOWN) || (BMT_status.charger_type == NONSTANDARD_CHARGER)||(BMT_status.charger_type == APPLE_0_5A_CHARGER))
    {
        battery_charging_control(CHARGING_CMD_GET_CHARGER_TYPE,&CHR_Type_num);
        BMT_status.charger_type = CHR_Type_num;	
    }	
...
#else        
		/*[M]fix media device cannot be detected when usb inserted*/
		if(( (BMT_status.charger_type == CHARGER_UNKNOWN) ||(BMT_status.charger_type == NONSTANDARD_CHARGER)||(BMT_status.charger_type == APPLE_0_5A_CHARGER)) &&
		   (g_charger_type_check_cnt>0))//xmxl@20140417: check charger type ;
		/*[M]end, bugid 15749, xmtzqh, 20131204*/
		{
		        g_charger_type_check_cnt--;//xmxl@20140417: check charger type ;
			mt_charger_type_detection();
...
void do_chrdet_int_task(void)
{
    if(g_bat_init_flag == KAL_TRUE)
    {
		...
		g_charger_type_check_cnt=G_CHARGER_TYPE_CHECK_MAX;//xmxl@20140417: check charger type
...
int bat_thread_kthread(void *x)
{
    ...
    while (1) {               
		...
		/*when detect to NONSTANDARD_CHARGER or APPLE_0_5A_CHARGER, cut down period, by xmyyq 20140514*/
		if((upmu_is_chr_det() == KAL_TRUE) && (g_charger_type_check_cnt > 0) &&
				((BMT_status.charger_type == CHARGER_UNKNOWN) || (BMT_status.charger_type == NONSTANDARD_CHARGER) || (BMT_status.charger_type == APPLE_0_5A_CHARGER)))

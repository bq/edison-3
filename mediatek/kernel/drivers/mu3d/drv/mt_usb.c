...
void connection_work(struct work_struct *data)
{
	...
	static enum status dev_status = INIT;
	...
	os_printk(K_INFO, "%s musb %s, cable %s\n", __func__, ((dev_status==0)?"INIT":((dev_status==1)?"ON":"OFF")), (is_usb_cable?"IN":"OUT"));

	if ((is_usb_cable == true) && (dev_status != ON) && (musb->usb_mode == CABLE_MODE_NORMAL)) {
	...
	} else if (((is_usb_cable == false) && (dev_status != OFF)) || (musb->usb_mode != CABLE_MODE_NORMAL)) {

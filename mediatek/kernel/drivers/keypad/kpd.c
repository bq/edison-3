...
#define CUST_EINT_MHALL_POLARITY CUST_EINT_MHALL_TYPE
...
static u8 kpd_hallkey_state = !CUST_EINT_MHALL_POLARITY;
#endif
/* for keymap handling */
...
static void kpd_hallkey_handler(unsigned long data)
{
    ...
    printk("add xmwuwh kpd_hallkey_state=%d,pressed=%d\n",kpd_hallkey_state,pressed);
	kpd_hallkey_state = !kpd_hallkey_state;
	pressed = (kpd_hallkey_state == !!CUST_EINT_MHALL_POLARITY);
	...
	if(!kpd_input_dev) {
        ...
        printk("add xmwuwh keycode=%d,pressed=%d\n",keycode,pressed);
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	mt_eint_registration(CUST_EINT_MHALL_NUM, CUST_EINT_MHALL_POLARITY,kpd_hallkey_eint_handler, false);
#endif

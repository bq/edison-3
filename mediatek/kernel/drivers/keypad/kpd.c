...
#ifdef CUSTOM_KERNEL_KPD_MHALL
static void kpd_hallkey_handler(unsigned long data);
static DECLARE_TASKLET(kpd_hallkey_tasklet, kpd_hallkey_handler, 0);
static u8 kpd_hallkey_state = !CUST_EINT_POLARITY_HIGH;
#endif
/*[END] xmxkp,20140526*/
...
static void kpd_hallkey_handler(unsigned long data)
{
	...
	pressed = (kpd_hallkey_state == !!CUST_EINT_POLARITY_HIGH);
	...
	if(!kpd_input_dev) {
		printk(KPD_SAY "kpd_hallkey_handler KPD input device not ready\n");
	}
    else{
		//kpd_backlight_handler(pressed, (pressed ? KEY_HALL_SLEEP : KEY_HALL_WAKE));
		keycode=(pressed ?  KEY_HALL_WAKE : KEY_HALL_SLEEP);
		input_report_key(kpd_input_dev, keycode, 1);
		input_sync(kpd_input_dev);
		input_report_key(kpd_input_dev, keycode, 0);
		input_sync(kpd_input_dev);
		kpd_print("report Linux keycode = %u\n", keycode);
    }
	/* for detecting the return to old_state */
	mt_eint_set_polarity(KPD_HALLKEY_EINT_NUM, old_state);
	mt_eint_unmask(KPD_HALLKEY_EINT_NUM);
}

static void kpd_hallkey_eint_handler(void)
{
    //printk(KPD_SAY "kpd_hallkey_eint_handler\n");
    tasklet_schedule(&kpd_hallkey_tasklet);
}
#endif
...
/*[A] add Hall-Switch function,xmxkp,20140526*/
#ifdef CUSTOM_KERNEL_KPD_MHALL
    __set_bit(KEY_HALL_SLEEP, kpd_input_dev->keybit);
    __set_bit(KEY_HALL_WAKE, kpd_input_dev->keybit);
#endif
/*[END] xmxkp,20140526*/
...
/*[A] add Hall-Switch function,xmxkp,20140526*/
#ifdef CUSTOM_KERNEL_KPD_MHALL
	mt_set_gpio_dir(GPIO_MHALL_EINT_PIN,0);// GPIO_DIR_IN
	mt_set_gpio_mode(GPIO_MHALL_EINT_PIN, GPIO_MHALL_EINT_PIN_M_EINT);
	mt_set_gpio_pull_enable(GPIO_MHALL_EINT_PIN, TRUE);
	mt_set_gpio_pull_select(GPIO_MHALL_EINT_PIN,1);// GPIO_PULL_UP

	mt_eint_set_polarity(KPD_HALLKEY_EINT_NUM, CUST_EINT_POLARITY_HIGH);
	mt_eint_set_hw_debounce(KPD_HALLKEY_EINT_NUM, KPD_HALLKEY_EINT_DEBOUNCE_CN);
	mt_eint_registration(KPD_HALLKEY_EINT_NUM, KPD_HALLKEY_EINT_DEBOUNCE_EN,kpd_hallkey_eint_handler, false);
#endif
/*[END] xmxkp,20140526*/

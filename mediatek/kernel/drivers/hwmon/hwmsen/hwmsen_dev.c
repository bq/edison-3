	...
	{		
			...
			||(sensor == ID_PROXIMITY) || (sensor == ID_TEMPRERATURE))
		{
			...
			if(data->values[0] != obj_data.sensors_data[sensor].values[0])
			{
				...
				if(sensor == ID_PROXIMITY){//xmxl@20140527:send status to app
				obj_data.sensors_data[sensor].status = data->status;
				//HWM_LOG("get %d sensor, values: %d, status=%d!\n", sensor, obj_data.sensors_data[sensor].values[0],obj_data.sensors_data[sensor].status);
				}
...
static int hwmsen_set_delay(int delay, int handle )
{
	...
	else if(atomic_read(&cxt->enable) != 0)
	{
		...
		#else
		if((err=cxt->obj.sensor_operate(cxt->obj.self, SENSOR_DELAY, &delay,sizeof(int), NULL, 0, NULL)) != 0)
		{
			HWM_ERR("%d sensor's sensor_operate function error %d,set defualt 200!\r\n",handle,err);
			atomic_set(&cxt->delay, 200);
			return -1;//xmxl@20140523:return result value from sensor driver
		}
		#endif
...
static long hwmsen_unlocked_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		case HWM_IO_SET_DELAY:
			...
			#else
			if(!(hwmsen_set_delay(delayPara.delay,delayPara.handle)<0))//xmxl@20140523
				update_workqueue_polling_rate(delayPara.delay);
			else{
				HWM_ERR("ioctl delay fails, because sensor driver doesn't support,set defualt 200 now!!\n");

...
int port_ipc_kernel_open(unsigned int task_id)
{
	...
	CCCI_INF_MSG(port->modem->index, CHAR, "IPC port %s open by %s\n", port->name, current->comm);
...
void port_ipc_kernel_read(struct ccci_port *port)
{
	...
	p_ilm = (struct ccci_ipc_ilm*)(req->skb->data);
	p_ilm->dest_mod_id = ccci_h->reserved;
	p_ilm->local_para_ptr = (struct local_para *)(p_ilm+1);
#if 0
	mtk_conn_md_bridge_send_msg(p_ilm);
#endif
	ccci_free_req(req);
}

int port_ipc_kernel_write(struct ccci_ipc_ilm *in_ilm)
{
	u32 task_id;
	...
	if(req) {
		...
		memcpy(skb_put(req->skb, sizeof(struct ccci_ipc_ilm)), in_ilm, sizeof(struct ccci_ipc_ilm));
		count = in_ilm->local_para_ptr->msg_len;
		memcpy(skb_put(req->skb, count), in_ilm->local_para_ptr, count);
		ilm = (struct ccci_ipc_ilm *)(ccci_h+1);

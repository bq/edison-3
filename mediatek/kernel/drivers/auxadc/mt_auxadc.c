...
//[A]Get hardware version,ArthurLin,20130819
#ifdef MALATA_HW_VERSION
...
static DEVICE_ATTR(malata_hw_version, 0664, show_malata_hw_version, NULL);
#endif
/*[A] END,ArthurLin,20130819 */

#if defined(MALATA_MODEM_VERSION)
...
#if defined(MALATA_E6901) || defined(MALATA_E8004) || defined(MALATA_E8006)
		if((value == 0 || value < 40)){
			ver = 1;
...
static DEVICE_ATTR(AUXADC_modem_baseband, 0664, show_AUXADC_read_modem_baseband, NULL);
#endif
...
/*[A] Get hardware version,ArthurLin,20130819 */
#ifdef MALATA_HW_VERSION
    if ((ret = device_create_file(&(dev->dev), &dev_attr_malata_hw_version)) != 0) goto exit;
#endif

#if defined(MALATA_MODEM_VERSION)
    if ((ret = device_create_file(&(dev->dev), &dev_attr_AUXADC_modem_baseband)) != 0) goto exit;
#endif

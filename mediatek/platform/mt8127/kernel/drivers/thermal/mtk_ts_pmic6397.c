...
static int mtktspmic_read_log(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
	int len = 0;
	char *p = buf;
...
static ssize_t mtktspmic_write_log(struct file *file, const char *buffer, unsigned long count, void *data)
{
	char desc[32];
	int log_switch;
	int len = 0;
...
static int __init mtktspmic_init(void)
{
	...
	else
	{
		...
		entry = create_proc_entry("mtktspmic_log", S_IRUGO | S_IWUSR, mtktspmic_dir);
		if (entry)
		{
			entry->read_proc = mtktspmic_read_log;
			entry->write_proc = mtktspmic_write_log;
		}
	}

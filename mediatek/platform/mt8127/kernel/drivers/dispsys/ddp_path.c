...
static int disp_reg_backup(void)
{
	...
	else
	{
		...
		//pr_err("Backup OVL reg in TEE");
	}
#endif // CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
	//DISP_MSG("2. reg_offset=%d \n",reg_offset);
...
static int disp_reg_restore(void)
{
    ...
    // BLS
    //disp_bls_restore();
	//DISP_MSG("1R. reg_offset=%d  ,  bls_en=0x%08x \n",reg_offset, DISP_REG_GET(DISP_REG_BLS_EN));
    ...
    else
    {
		...
		//pr_err("Restore OVL reg in TEE");
    }
#endif // CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT

	//DISP_MSG("2R. reg_offset=%d  ,  bls_en=0x%08x \n",reg_offset,DISP_REG_GET(DISP_REG_BLS_EN));
    ...
    for (index = 0; index < 1; index++) {   //Donot restore RDMA1. It's only for HDMI using.
		...
		//DISP_MSG("3R. reg_offset=%d  ,  bls_en=0x%08x \n",reg_offset, DISP_REG_GET(DISP_REG_BLS_EN));
#ifdef CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
	...
	//pr_err("Restore WDMA reg in REE");
#ifdef CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
    }
    else
    {
		...
		//pr_err("Restore WDMA reg in TEE");
    }
#endif // CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
	//DISP_MSG("4R. reg_offset=%d ,  bls_en=0x%08x \n",reg_offset, DISP_REG_GET(DISP_REG_BLS_EN));
...
int disp_intr_disable_and_clear(void)
{
	...
	//pr_err("[DC]gOvlLayerSecure[0]=%d, gOvlLayerSecure[1]=%d, gOvlLayerSecure[2]=%d, gOvlLayerSecure[3]=%d",gOvlLayerSecure[0],gOvlLayerSecure[1],gOvlLayerSecure[2],gOvlLayerSecure[3]);
#ifdef CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
    if (0 == (gOvlLayerSecure[0]|gOvlLayerSecure[1]|gOvlLayerSecure[2]|gOvlLayerSecure[3]))
#endif
	{
    ...
    //DISP_REG_SET(DISP_REG_RDMA_INT_STATUS+0xa000, 0);
    DISP_REG_SET(DISP_REG_CONFIG_MUTEX_INTSTA, 0);

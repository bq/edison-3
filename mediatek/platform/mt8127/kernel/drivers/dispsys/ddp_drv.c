...
//	                      }
        ...
        //                        }
                        ...
                        // If failed to clear intr, the irq handler will occupy the CPU.
                        // It's found when RDMA1 clock is off, then disp_path_config_ is
                        // called in hdmi_rdma_address_config.
                        if ((reg_val & DISP_REG_GET(DISP_REG_RDMA_INT_STATUS+0xa000)) == reg_val)
                        {
                            printk("[irq] rdma1 error %llu\n", sched_clock());
                        }

...
void vRepeaterOnOff(unsigned char fgIsRep)
{
  ...
  if(fgIsRep == TRUE)
  {
    ...
    HDMI_DEF_LOG("[hdmi][hdcp]repeater mode\n");
  }
  else
  {
    bTemp &= ~HDCP_CTRL_RX_RPTR;
    HDMI_DEF_LOG("[hdmi][hdcp]receiver mode\n");
  }
...
void vSendAn(void) 
{
  ...
  HDMI_DEF_LOG("[hdmi][hdcp]an(0~7)%x,%x,%x,%x,%x,%x,%x,%x\n",bHDCPBuf[0],bHDCPBuf[1],bHDCPBuf[2],bHDCPBuf[3],bHDCPBuf[4],bHDCPBuf[5],bHDCPBuf[6],bHDCPBuf[7]);
}
...
void vExchangeKSVs(void)
{
  ...
  HDMI_DEF_LOG("[hdmi][hdcp]bksv(0~4)%x,%x,%x,%x,%x\n",bHDCPBuf[0],bHDCPBuf[1],bHDCPBuf[2],bHDCPBuf[3],bHDCPBuf[4]);
...
unsigned char fgCompareRi(void)
{
  ...
  fgDDCDataRead(RX_ID, RX_REG_RI, HDCP_RI_COUNT, bHDCPBuf);
    ...
    if(e_hdcp_ctrl_state == HDCP_COMPARE_R0)
        HDMI_DEF_LOG("[hdmi][hdcp]rx:r0[0]:0x%x,r0[1]:0x%x;tx:r0[0]:0x%x,r0[1]:0x%x\n",bHDCPBuf[0],bHDCPBuf[1],bHDCPBuf[2],bHDCPBuf[3]);
    else
        HDMI_HDCP_LOG("[hdcp]rx:R0[0]=0x%x,R0[1]=0x%x;tx:R0[0]=0x%x,R0[1]=0x%x\n",bHDCPBuf[0],bHDCPBuf[1],bHDCPBuf[2],bHDCPBuf[3]);
  // compare R0 and R0'
...
{
  HDMI_DEF_LOG("[hdmi][hdcp]init,_bHdcpOff:%d\n",_bHdcpOff);

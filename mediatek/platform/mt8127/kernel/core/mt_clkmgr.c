...
#if CLKMGR_8127
	p += sprintf(p, "set freq   : echo setfreq id freq_khz   > /proc/clkmgr/pll_test\n");
#endif /* CLKMGR_8127 */
				...
				int count, int *eof, void *data)
{
	...
	for (i = 0; i < NR_SYSS; i++) {
		...
		p += sprintf(p, "[%d][%-8s]=[0x%08x], state(%u)\n", i, name, value, state);
	}
...
static int sprint_mux(char *p, int mux_id)
{
	int reg_value;
	int mux_value;
	int mux_pdn;
	...
	return sprintf(p, "[ %2d, %2d, %3s, %-16s, %2d: %-12s ]\n",
				...
				int count, int *eof, void *data)
{
	...
	p += sprintf(p, "\n********** mux_test help *********\n");
	p += sprintf(p, "clkmux     : echo clkmux  mux_id val [mod_name] > /proc/clkmgr/mux_test\n");
	p += sprintf(p, "enable  mux: echo enable  mux_id     [mod_name] > /proc/clkmgr/mux_test\n");
	p += sprintf(p, "disable mux: echo disable mux_id     [mod_name] > /proc/clkmgr/mux_test\n");
...
static int mux_test_write(struct file *file, const char *buffer,
						  unsigned long count, void *data)
{
	char desc[32];
	int len = 0;
...
void mt_clkmgr_debug_init(void)
{
...
#if CLKMGR_8127
		entry->write_proc = mux_test_write;
#endif /* CLKMGR_8127 */
	}
	...
	if (entry) {
#if CLKMGR_8127
		entry->proc_fops = &clk_test_file_ops;
#else /* !CLKMGR_8127 */

...
static MINT32 __init ISP_Init(MVOID)
{
    ...
    pEntry = create_proc_entry("driver/cam_clk", 0, NULL);
    if(pEntry)
    {
        pEntry->read_proc = CAM_CLK_DumpRegToProc;
        pEntry->write_proc = CAM_CLK_RegDebug;
    }
    else
    {
        LOG_ERR("add /proc/driver/cam_clk entry fail");
    }

...
//{0x04, 0x05, 0x06, 0x07, 0x0D}
	mtk_nand_sprmset_rrtry(0x04, data_tbl[retryCount][0]);
	mtk_nand_sprmset_rrtry(0x05, data_tbl[retryCount][1]);
	mtk_nand_sprmset_rrtry(0x06, data_tbl[retryCount][2]);
	mtk_nand_sprmset_rrtry(0x07, data_tbl[retryCount][3]);
	mtk_nand_sprmset_rrtry(0x0D, data_tbl[retryCount][4]);
...
static void mtk_nand_parapage_rrtry(flashdev_info deviceinfo, u32 retryCount, bool defValue)
{
	//TODO
	#if 1
	mtk_nand_toshiba_rrtry(deviceinfo, retryCount, defValue);
	#endif
}
...
{
	u32 value;
	if(RTYPE_FEATURE == type || RTYPE_PRECMD== type)
	{

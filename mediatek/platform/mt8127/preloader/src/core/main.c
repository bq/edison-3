...
static void bldr_pre_process(void)
{
    ...
    /*During handshaking with Meta tool, log cannot be outputted in case of Meta UART port same with log UART port*/
    #if 0
    /* switch UART setting for log output */
...
#if CFG_MTK_IN_HOUSE_TEE_SUPPORT
extern int bldr_load_mtee_part_ex
(
blkdev_t* bdev,
u32*      pt_addr,
u32       u32_ext_mem_start_addr,
u32*      pu32_sz,
const u8    au8_pubk_n[MTEE_IMG_VFY_PUBK_SZ],
const char* apc_mtee_part_name[2]
);

/*  RSA2048 public key for verifying mtee image
    MTEE_IMG_VFY_PUBK is defined in alps\mediatek\custom\[project]\preloader\inc\cust_sec_ctrl.h
    It should be the same as AUTH_PARAM_N in alps\mediatek\custom\[project]\trustzone\cfg\MTEE_IMG_PROTECT_CFG.ini
*/
static const u8 l_mtee_img_vfy_pubk[MTEE_IMG_VFY_PUBK_SZ] = { MTEE_IMG_VFY_PUBK };

static int bldr_load_mtee_image(u32 *jump_addr)
{
    u32 u32_mem_sz;
    u32 u32_addr;
    blkdev_t *bootdev;    
    const char* apc_mtee_part_name[2] = { PART_TEE1, PART_TEE2 };

    if( NULL == jump_addr )
    {
        print("%s Error: NULL pointer for bootdev and jump address\n", MOD);
        return -1;    
    }
    ...
    u32_addr   = 0;
    u32_mem_sz = memory_size();
    
    /* check if external memory size is enough */
    if( u32_mem_sz <= MAX_TEE_DRAM_SIZE )
    {
       print("%s Error: external memory size (%d) is too small for loading mtee image\n", MOD, memory_size());
       return -3;
    }

    /* min addr for mtee image */ 	 
    u32_addr = CFG_DRAM_ADDR + u32_mem_sz - MAX_TEE_DRAM_SIZE;

    /* dram in [addr + mtee image size, addr + mtee image size + 128] will be used during mtee image loading */ 	 
    if(0 != bldr_load_mtee_part_ex(bootdev, &u32_addr, CFG_DRAM_ADDR, &u32_mem_sz, l_mtee_img_vfy_pubk, apc_mtee_part_name ))
    {
        print("%s Error: fail to load mtee image\n", MOD);
        return -4;
    }

    /* set jump address for preloader */
    *jump_addr = u32_addr;

    /* set adjust dram sz for boot args */
    g_adjust_dram_sz_for_boot_args = u32_mem_sz;
    return 0;
}
...
#if CFG_MTK_IN_HOUSE_TEE_SUPPORT
    // make sure no sec lib is used from here
    if (0 != bldr_load_mtee_image(&jump_addr)) {
        print("%s fail to load mtee image\n", MOD);
        goto error;
    }
#endif

...
ifeq (yes, $(MTK_IN_HOUSE_TEE_SUPPORT))
CFG_MTK_IN_HOUSE_TEE_SUPPORT := 1
else
CFG_MTK_IN_HOUSE_TEE_SUPPORT := 0
endif

define is_custom_dram_size_support
$(shell if   [ 2G = "$(CUSTOM_DRAM_SIZE)" ]; then echo 0x10000000;  \
        elif [ 4G = "$(CUSTOM_DRAM_SIZE)" ]; then echo 0x20000000;  \
        elif [ 6G = "$(CUSTOM_DRAM_SIZE)" ]; then echo 0x30000000;  \
        elif [ 8G = "$(CUSTOM_DRAM_SIZE)" ]; then echo 0x40000000;  \
        else echo 0; \
        fi)
endef
CFG_CUSTOM_DRAM_SIZE_SUPPORT :=$(call is_custom_dram_size_support)

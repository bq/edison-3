...
int mmc_block_read(int dev_num, unsigned long blknr, u32 blkcnt, void *dst, u32 part_id)
{
    struct mmc_host *host = mmc_get_host(dev_num);
    struct mmc_card *card = mmc_get_card(dev_num);
    u32 blksz    = host->blklen;
...
int mmc_block_write(int dev_num, unsigned long blknr, u32 blkcnt, const void *src, u32 part_id)
{
    struct mmc_host *host = mmc_get_host(dev_num);
    struct mmc_card *card = mmc_get_card(dev_num);
    u32 blksz    = host->blklen;
...
{
    return mmc_block_read(dev_num, blknr, blkcnt, dst, part_id) == MMC_ERR_NONE ? blkcnt : (unsigned long) -1;
}
unsigned long mmc_wrap_bwrite(int dev_num, unsigned long blknr, lbaint_t blkcnt, const void *src, u32 part_id)
{
    return mmc_block_write(dev_num, blknr, blkcnt, src, part_id) == MMC_ERR_NONE ? blkcnt : (unsigned long) -1;
}
#if 0
...
__attribute__ ((aligned(64))) static uchar sd_header_buf[512];
#define FACTORY_STRING "enter factory mode"
extern BOOTMODE g_boot_mode;

int sd_legacy_init(int verbose)
{
    int id = verbose - 1;
    int err = MMC_ERR_NONE;
    struct mmc_host *host;
    ...
    if (err == MMC_ERR_NONE) {
		...
		//Now start read the first block of SD card
		memset(sd_header_buf, 0, 512);
		bdev->block_read(id, 0x10, 1, (void *)sd_header_buf, 0);
		if (!strcmp((const char *)sd_header_buf, FACTORY_STRING)) {
			printf("jct debug: now enter factory mode!\n");

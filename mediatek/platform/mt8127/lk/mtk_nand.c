...
static void mtk_nand_sprmset_rrtry(u32 addr, u32 data) //single parameter setting
{
	...
	nand_status_ready(STA_NAND_BUSY_RETURN);
...
#if 0
struct rrtry_setting{
	u32 setting[30];
};

struct rrtry_setting special_rrtry_setting[10]= 
{
	{0},{0},{0},{0},
...
static u32 mtk_nand_rrtry_setting(flashdev_info deviceinfo, enum readRetryType type, u32 retryStart, u32 loopNo)
{
	...
	if(RTYPE_FEATURE == type || RTYPE_PRECMD== type)
	{
		...
		{
			value = special_rrtry_setting[deviceinfo.vendor].setting[loopNo];
		}
	}

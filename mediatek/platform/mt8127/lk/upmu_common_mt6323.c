...
void upmu_set_vgp1_tp_poweron(void)
{
  kal_uint32 ret=0;

  pmic_lock();
  ret=pmic_config_interface(DIGLDO_CON28, 0x5, PMIC_RG_VGP1_VOSEL_MASK, PMIC_RG_VGP1_VOSEL_SHIFT);
  pmic_unlock();
  if(ret!=0) printf("%d", ret);
  pmic_lock();
  ret=pmic_config_interface(DIGLDO_CON7, 0x1, PMIC_RG_VGP1_EN_MASK, PMIC_RG_VGP1_EN_SHIFT);
  pmic_unlock();
  if(ret!=0) printf("%d", ret);
}

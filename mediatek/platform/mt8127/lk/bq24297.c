...
U32 bq24297_i2c_read (U8 chip, U8 *cmdBuffer, int cmdBufferLen, U8 *dataBuffer, int dataBufferLen)
{
    U32 ret_code = I2C_OK;
    	
    ret_code = mt_i2c_write(BQ24297_BUSNUM, chip, cmdBuffer, cmdBufferLen, 1);	 // set register command
    if (ret_code != I2C_OK)
    return ret_code;
    
    ret_code = mt_i2c_read(BQ24297_BUSNUM, chip, dataBuffer, dataBufferLen, 1);
    	
    //dprintf(CRITICAL, "[bq24297_i2c_read] Done\n");
    	
    return ret_code;
}
	
U32 bq24297_i2c_write (U8 chip, U8 *cmdBuffer, int cmdBufferLen, U8 *dataBuffer, int dataBufferLen)
{
    U32 ret_code = I2C_OK;
    ...
    {
        dprintf(CRITICAL, "[bq24297_i2c_write] exceed I2C FIFO length!! \n");
        return 0;
    }
    ...
    {
        //dprintf(CRITICAL, "[bq24297_i2c_write] write_data[%d]=%x\n", i, write_data[i]);
    }
    	
    ret_code = mt_i2c_write(BQ24297_BUSNUM, chip, write_data, transfer_len, 1);
    	
    //dprintf(CRITICAL, "[bq24297_i2c_write] Done\n");
...
{
    U8 chip_slave_address = bq24297_SLAVE_ADDR_WRITE;
    U8 cmd = 0x0;
    int cmd_len = 1;
    U8 data = 0xFF;
    int data_len = 1;
    ...
    cmd = RegNum;
    result_tmp = bq24297_i2c_read(chip_slave_address, &cmd, cmd_len, &data, data_len);

    //dprintf(INFO, "[bq24297_read_interface] Reg[%x]=0x%x\n", RegNum, data);
    
    data &= (MASK << SHIFT);
    *val = (data >> SHIFT);
    
    //dprintf(INFO, "[bq24297_read_interface] val=0x%x\n", *val);

    if(g_bq24297_log_en>1)        
        dprintf(CRITICAL, "%d\n", result_tmp);
...
kal_uint32 bq24297_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    cmd = RegNum;
    result_tmp = bq24297_i2c_read(chip_slave_address, &cmd, cmd_len, &data, data_len);
    //dprintf(INFO, "[bq24297_config_interface] Reg[%x]=0x%x\n", RegNum, data);

    data &= ~(MASK << SHIFT);
    data |= (val << SHIFT);

    result_tmp = bq24297_i2c_write(chip_slave_address, &cmd, cmd_len, &data, data_len);
    //dprintf(INFO, "[bq24297_config_interface] write Reg[%x]=0x%x\n", RegNum, data);

    // Check
    result_tmp = bq24297_i2c_read(chip_slave_address, &cmd, cmd_len, &data, data_len);
    //dprintf(INFO, "[bq24297_config_interface] Check Reg[%x]=0x%x\n", RegNum, data);
...
void bq24297_dump_register(void)
{
    ...
    {
        cmd = i;
        bq24297_i2c_read(chip_slave_address, &cmd, cmd_len, &data, data_len);
        bq24297_reg[i] = data;
        dprintf(CRITICAL, "[bq24297_dump_register] Reg[0x%X]=0x%X\n", i, bq24297_reg[i]);
    }	
}

void bq24297_hw_init(void)
{
    ...
    if(0)
	    bq24297_set_sys_min(0x0); //Minimum system voltage 3.0V (MT6320 E1 workaround, disable powerpath)
    else	    
...
void bq24297_turn_on_charging(void)
{
    bq24297_set_en_hiz(0x0);	        	
    bq24297_set_chg_config(0x1); // charger enable
    printf("[BATTERY:bq24297] charger enable !\r\n");
}

...
status_t AudioAfeReg::SetApReg(uint32 address, uint32 value, uint32 mask)
{
    mReg_Control.offset = address;
    mReg_Control.value = value;
    mReg_Control.mask = mask;
    ::ioctl(mFd, SET_AP_REG, &mReg_Control);

    return NO_ERROR;
}

uint32_t AudioAfeReg::GetApReg(uint32 address)
{
    // here need to get afe ref base on address
    mReg_Control.offset = address;
    mReg_Control.value = 0;
    mReg_Control.mask = 0xffffffff;
    ::ioctl(mFd, GET_AP_REG, &mReg_Control);

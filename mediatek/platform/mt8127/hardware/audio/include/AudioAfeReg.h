...
//#define AFE_MEMIF_MINLEN        (0x03D0)  // removed
...
class AudioAfeReg
{
    public:
        ...
        */
        ...
        //The following is only for debug purpose, please don't use directly in audio HAL code
        /**
        * a basic function to set AP regiseter (kernel virtual address, for debug purpose)
        * @param address - kernel virtual address
        * @param value - value want to set to register (32bits)
        * @param mask - mask with 32 bits regiseter
        * @see GetApReg()
        * @return The state of set regiseter
        */
        status_t SetApReg(uint32 address, uint32 value, uint32 mask);

        /**
        * a basic function to Get AP regiseter (kernel virtual address, for debug purpose)
        * @param address - kernel virtual address
        * @see SetApReg()
        * @return The value regiseter want to get
        */
        uint32 GetApReg(uint32 address);

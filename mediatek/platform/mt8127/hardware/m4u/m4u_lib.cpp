...
//for only new flow. current may not work.
//UREE_AllocSecurechunckmem
M4U_STATUS_ENUM MTKM4UDrv::m4u_alloc_mva_sec(M4U_MODULE_ID_ENUM eModuleID,
                                const unsigned int SecMemHandle,
                                ...
                                unsigned int *pRetMVABuf)
{
    M4U_MOUDLE_STRUCT m4u_module;
    m4u_module.eModuleID = eModuleID;
    m4u_module.BufAddr = SecMemHandle;
    m4u_module.BufSize = MemSize;
    m4u_module.security = 1;
    m4u_module.cache_coherent = cache_coherent;
    ...
    if(m4u_check_m4u_en(eModuleID))
    {
        ...
        if(mFileDescriptor<0 )
        {
            M4UMSG("m4u_alloc_mvasec failed \n");
            m4u_dump_backtrace();
            return M4U_STATUS_INVALID_HANDLE;
        }
        if(ioctl(mFileDescriptor, MTK_M4U_T_ALLOC_MVA_SEC, &m4u_module))
        {
            M4UMSG(" ioctl MTK_M4U_T_ALLOC_MVA sec fail! %d, %s \n", errno, strerror(errno));
            m4u_dump_backtrace();
            return M4U_STATUS_KERNEL_FAULT;
        }
        else
        {
            *pRetMVABuf = m4u_module.MVAStart;
            return M4U_STATUS_OK;
        }
    }
    else
...
M4U_STATUS_ENUM MTKM4UDrv::m4u_dealloc_mva_sec(M4U_MODULE_ID_ENUM eModuleID,
                                const unsigned int SecMemHandle,
                                const unsigned int MemSize,
                                const unsigned int MVAHandle)
{
    ...
    if(mFileDescriptor<0 )
    {
        M4UMSG("m4u_dealloc_mvasec failed \n");
        m4u_dump_backtrace();
        return M4U_STATUS_INVALID_HANDLE;
    }
    if(ioctl(mFileDescriptor, MTK_M4U_T_DEALLOC_MVA_SEC, &m4u_module))
    {
        M4UMSG(" ioctl MTK_M4U_T_DEALLOC_MVA sec fail! %d, %s \n", errno, strerror(errno));
        m4u_dump_backtrace();
        return M4U_STATUS_KERNEL_FAULT;
    }
    else

...
#ifdef MALATA_E7006
#define MALATA_HW_VER1 
#else

#endif
int get_hw_version_adcaux(int channel)
{
    int value=0, data[4] = {0,0,0,0};
    int ret = 0, hw_version = 0, times = 0;
    unsigned int time_read_adcaux = 0;
    time_read_adcaux = get_timer(0);

    printf("[adc_driver]:get_hw_version_adcaux:channel=%d\n", channel);//xmlq debug

	/* Get adcaux one value */
    /*
	//IMM_GetOneChannelValue(channel, data, 0);
    value = IMM_auxadc_get_evrage_data();
    if(ret < 0)
    {
    ...
    */
    ...
    /* Get adcaux evrage value in 5 times*/
    while (times < 5)
    {
        ret = IMM_GetOneChannelValue(channel, data, 0);
        hw_version = 1;
        if(ret < 0)
        {
            printf("[adc_driver]: get data error\n");
...
//		msleep(10);
    ...
    if ((value < 100) && (value > 85))
        hw_version = 1;
    time_read_adcaux = get_timer(time_read_adcaux);
    printf("[adc_driver]: get hw_version adc takes %d ms \n", time_read_adcaux);//xmlq debug
    printf("[adc_driver]: auxadc=%d, hw_version=%d \n", value, hw_version);

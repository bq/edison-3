...
start()
{
    ...
    /*[M] fix video talk main camera orientation error bug_id=0026706 xmcwy 20140709 */
    #if (defined(CUSTOMER_Q5) || defined(CUSTOMER_BLU)||defined(CUSTOMER_Q18) || (defined(MALATA_E1002) && defined(CUSTOMER_ES))) 
    halSensorDev_e temp_sensor_id;
    temp_sensor_id = (halSensorDev_e)DevMetaInfo::queryHalSensorDev(gInfo.openId);
    if(temp_sensor_id == SENSOR_DEV_MAIN){
	dum_ang = 90;
	MY_LOGD("xmcwy_camera_vt temp_sensor_id == SENSOR_DEV_MAIN dum_ang = %d \n", dum_ang);
    }else{
    ...
    dum_ang = get_dummy_angle();
    #endif
    /*[E] fix video talk main camera orientation error bug_id=0026706 xmcwy 20140709 */

...
uint32_t AudioPreProcess::WriteEchoRefData(void *buffer , uint32_t bytes)
{
    AudioAutoTimeoutLock _l(mLock);
    if (mEcho_Reference != NULL)
    {
        ...
        b.raw = (void *)buffer;
        b.frame_count = bytes / sizeof(int16_t) / mStreamInAttribute->num_channels;
                                                                  ...
                                                                  uint32_t channel_count, uint32_t sampling_rate)
{
...

    {
        ...
        uint32_t wr_channel_count = channel_count;

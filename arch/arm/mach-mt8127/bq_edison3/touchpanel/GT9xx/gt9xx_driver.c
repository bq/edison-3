...
int tpd_halt = 0;
int tpd_det = 0;	//tpd_det = 0 :tp is gt9110;1 :tp is gt928
...
#if GTP_GESTURE_CTL_FILENODE
static struct class *tp_ctl_class = NULL;
int double_tap_switch;
#endif
...
s32 gtp_read_version(struct i2c_client *client, u16 *version)
{
    ...
    if ((buf[3] == 0x32)&&(buf[4] == 0x38))
    {	
    	tpd_det = 1;
	GTP_INFO("tpd_det = %d,gt928 is chosed\n",tpd_det);
    }
    else
    {
	tpd_det = 0;
	GTP_INFO("tpd_det = %d,gt9110 is chosed\n",tpd_det);
    }	
...
#if defined MALATA_E7007 || defined MALATA_Q5 || defined MALATA_D7023
        input_report_abs(tpd->dev, ABS_MT_POSITION_X, 800-x);
        input_report_abs(tpd->dev, ABS_MT_POSITION_Y, 1280-y);
#else
...
{
    u32 chr_status = 0;
	u8 chr_cmd1[3] = {0x80, 0x46};
    u8 chr_cmd[3] = {0x80, 0x40};
    static u8 chr_pluggedin = 0;
    int ret = 0;
    //printk("yemk gtp_charger_switch\n");
#ifdef MT6573
    ...
    if (chr_status)     // charger plugged in
    {
        if (!chr_pluggedin || dir_update)
        {
			...
			ret = gtp_i2c_write(i2c_client_point, chr_cmd1, 3);
            if (ret > 0)
            {
...
static ssize_t double_tap_wakeup_write(struct class *cls, struct class_attribute *attr, const char *_buf, size_t _count)
{
	...
	if (value != 0 && value != 1)
	{
		printk("double_tap_wakeup_write, invalid value.\n");
		return _count;
	}
	else
	{
		double_tap_switch = value;
	}
	printk("double_tap_wakeup has been writed.\n");
	return _count;
	}
...
static int tpd_local_init(void)
{
...
#if GTP_GESTURE_CTL_FILENODE
    {
        int retval = 0;
        double_tap_switch = 0;
        tp_ctl_class = class_create(THIS_MODULE, "ft5606");
        if (tp_ctl_class == NULL)
        {
            printk("create class ft5x06 failed!\n");
            return -1;
        }
        retval = class_create_file(tp_ctl_class, &class_attr_double_tap_wakeup);
        if (retval != 0)
        {
            printk("create ft5x06 class file failed!\n");

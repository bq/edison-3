...
void mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
	...
	char *br_ptr;
		...
		if ((br_ptr = strstr(*cmdline, "boot_reason=")) != 0) {
			...
			printk("%s:g_boot_reason=%d \n", __func__, g_boot_reason);
		}

    /*[A] Get hardware version,ArthurLin,20130819 */
    #ifdef CONFIG_MALATA_HW_VERSION
    if ((br_ptr = strstr(*cmdline, "hw_version=")) != 0) {
        /* get_hw_version */
        g_hw_version = br_ptr[11] - '0';
        printk("%s:g_hw_version=%d \n", __func__, g_hw_version);
    }
    #endif

    /*[A] Get hardware version,ArthurLin,20130819 */
    #ifdef CONFIG_MALATA_LCD_ID
    if ((br_ptr = strstr(*cmdline, "lcd_id=")) != 0) {
        g_lcd_id = br_ptr[7] - '0';
        printk("%s:g_lcd_id=%d \n", __func__, g_lcd_id);
    }
    #endif

...
#define POWER_NONE_MACRO MT65XX_POWER_NONE //add for liangkaiyang
...
static struct i2c_board_info __initdata i2c_BMA250={ I2C_BOARD_INFO(BMA250_DEV_NAME, (BMA250_I2C_SLAVE_WRITE_ADDR>>1))};
...
/*----------------------------------------------------------------------------*/
	...
	//	int sbit = 4;
		...
		{
			data[BMA250_AXIS_X] = (s16)((buf[BMA250_AXIS_X*2]>>6) | (buf[BMA250_AXIS_X*2+1]<<2) );
			data[BMA250_AXIS_Y] =(s16)((buf[BMA250_AXIS_Y*2]>>6) | (buf[BMA250_AXIS_Y*2+1]<<2) );
			data[BMA250_AXIS_Z] = (s16)((buf[BMA250_AXIS_Z*2]>>6) | (buf[BMA250_AXIS_Z*2+1]<<2) );	
			...
			if(data[BMA250_AXIS_X]&0x200)
			{
					data[BMA250_AXIS_X] = ~data[BMA250_AXIS_X];
					data[BMA250_AXIS_X] &= 0x1ff;
					data[BMA250_AXIS_X]+=1;
					data[BMA250_AXIS_X] = -data[BMA250_AXIS_X];
			}
		//	data[BMA250_AXIS_X] = data[BMA250_AXIS_X] / sbit;

			
			if(data[BMA250_AXIS_Y]&0x200)
			{
					data[BMA250_AXIS_Y] = ~data[BMA250_AXIS_Y];
					data[BMA250_AXIS_Y] &= 0x1ff;
					data[BMA250_AXIS_Y]+=1;
					data[BMA250_AXIS_Y] = -data[BMA250_AXIS_Y];
			}
		//	data[BMA250_AXIS_Y] = data[BMA250_AXIS_Y] / sbit;
			
			if(data[BMA250_AXIS_Z]&0x200)
			{
					data[BMA250_AXIS_Z] = ~data[BMA250_AXIS_Z];
					data[BMA250_AXIS_Z] &= 0x1ff;
					data[BMA250_AXIS_Z]+=1;
					data[BMA250_AXIS_Z] = -data[BMA250_AXIS_Z];
			}
		//	data[BMA250_AXIS_Z] = data[BMA250_AXIS_Z] / sbit;
	        ...
	        #if 0 //old process code
			data[BMA250_AXIS_X] = (s16)buf[BMA250_AXIS_X*2] ;
			data[BMA250_AXIS_Y] = (s16)buf[BMA250_AXIS_Y*2];
			data[BMA250_AXIS_Z] = (s16)buf[BMA250_AXIS_Z*2] ;
			if(atomic_read(&priv->trace) & ADX_TRC_RAWDATA)
			{
				GSE_LOG("[%08X %08X %08X] => [%5d %5d %5d] before\n", data[BMA250_AXIS_X], data[BMA250_AXIS_Y], data[BMA250_AXIS_Z],
										   data[BMA250_AXIS_X], data[BMA250_AXIS_Y], data[BMA250_AXIS_Z]);
			...
			if(data[BMA250_AXIS_X]&0x80)
			{
					data[BMA250_AXIS_X] = ~data[BMA250_AXIS_X];
					data[BMA250_AXIS_X] &= 0xff;
					data[BMA250_AXIS_X]+=1;
					data[BMA250_AXIS_X] = -data[BMA250_AXIS_X];
			}
			if(data[BMA250_AXIS_Y]&0x80)
			{
					data[BMA250_AXIS_Y] = ~data[BMA250_AXIS_Y];
					data[BMA250_AXIS_Y] &= 0xff;
					data[BMA250_AXIS_Y]+=1;
					data[BMA250_AXIS_Y] = -data[BMA250_AXIS_Y];
			}
			if(data[BMA250_AXIS_Z]&0x80)
			{
					data[BMA250_AXIS_Z] = ~data[BMA250_AXIS_Z];
					data[BMA250_AXIS_Z] &= 0xff;
					data[BMA250_AXIS_Z]+=1;
					data[BMA250_AXIS_Z] = -data[BMA250_AXIS_Z];
			}
...
static int BMA250_SetBWRate(struct i2c_client *client, u8 bwrate)
{
	...
	printk("BMA250_SetBWRate OK!--liang \n");
...
//       unsigned long arg)
static long bma250_unlocked_ioctl(struct file *file, unsigned int cmd,
       unsigned long arg)
...
static int bma250_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = bma250_gsensor_operate;
...
static int bma250_local_init(void)
{
    ...
    {
        GSE_ERR("bma250 prob and init error.\n");
        return -1;
    }
...
{
	GSE_FUN();
	printk("250bma-liangkaiyang\n");
	i2c_register_board_info(2, &i2c_BMA250, 1);
